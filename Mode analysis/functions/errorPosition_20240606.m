function [dwp_mat, dwm_mat, other] = errorPosition_20240606(U, Xref, K, Wp, Wm, M, tracking_error)
    ...
    for l = 1:size(K,1)
        ...
        for k = 1:tau % t
            dD_du_mat(i,j,k,l,ii,jj) = 1 / N / tau *...
                ( delta_bg * sum(exp(Imag * dXref_temp * K(l,:)') .* U(:,ii,k))  + ...
                  delta_ag * sum(exp(Imag * -dXref_temp * K(l,:)') .* U(:,jj,k)));
        end
    end; end; end; end; end
    % toc
    ...
    for k = 1:2 % a
        ...
        for l = 1:tau
            part1_t_temp = (dD_du_mat(j,k,l,i,1,1) + dD_du_mat(j,k,l,i,2,2));
            part2_t_temp = (D_mat(i,2,2) * dD_du_mat(j,k,l,i,1,1) + ...
                            D_mat(i,1,1) * dD_du_mat(j,k,l,i,2,2) - ...
                            D_mat(i,1,2) * dD_du_mat(j,k,l,i,2,1) - ...
                            D_mat(i,2,1) * dD_du_mat(j,k,l,i,1,2));
